...
export default function JobsFilter({
  ...
  const filtered = useMemo(() => {
    ...
    jobs = [...jobs].sort((a, b) => {
      ...
      if (sort === 'candidaturas') return b.id.localeCompare(a.id); // determinístico por id
      return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
    });

    return jobs;
  }, [initialJobs, query, sector, neighborhood, contract, sort]);

  const hasFilters = query || sector || neighborhood || contract;
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 border rounded-lg text-sm font-medium transition-colors ${showFilters ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-200 text-gray-700 hover:bg-gray-50'}`}
        >
          <i className="ri-filter-3-line"></i>
          Filtros
          {hasFilters && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>}
        </button>
        <select
          value={sort}
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            ...
            <select
              ...
              className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-700 outline-none bg-white"
            >
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-sm mt-1">Tente outros filtros ou palavras-chave</p>
        </div>
      ) : (
