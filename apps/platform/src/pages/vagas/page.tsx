...
// Deterministic mock candidature count based on job id
...
export default function VagasPage() {
  ...
  const [selectedNeighborhood, setSelectedNeighborhood] = useState(searchParams.get("bairro") || "Todos");
  ...
  // Count pending filters (to enable "Buscar Vagas" button)
  ...
  const filtered = useMemo(() => {
    const base = mockJobs.filter((job) => {
      ...
      return matchSearch && matchArea && matchContract && matchNeighborhood && matchSector;
    });
    ...
    return [...base].sort((a, b) => {
      ...
      if (sortBy === "candidaturas") {
        return mockCandidatures(b.id) - mockCandidatures(a.id);
      }
      return 0;
    });
  }, [search, selectedArea, selectedContract, selectedNeighborhood, selectedSector, sortBy]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-emerald-900 pt-24 pb-12">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              onClick={openFilters}
              className="flex items-center gap-2 bg-white/10 border border-white/20 text-white px-4 py-3 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap hover:bg-white/20 transition-colors"
            >
      ...
      {showFilters && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end sm:justify-center sm:items-center">
          ...
          <div className="relative bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl shadow-xl z-10 flex flex-col max-h-[90vh]">
            ...
            {/* Panel Footer — Buscar Vagas button */}
            <div className="px-5 py-4 border-t border-gray-100 bg-white rounded-b-2xl">
              ...
              <button
                ...
                className={`w-full py-3 rounded-xl text-sm font-semibold transition-all whitespace-nowrap ${
                  pendingFilterCount > 0
                    ? "bg-emerald-600 text-white cursor-pointer hover:bg-emerald-700 active:scale-95"
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center gap-2 overflow-x-auto scrollbar-hide">
          {ALL_SECTORS.map((s) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap cursor-pointer transition-colors border ${
                selectedSector === s
                  ? "bg-emerald-600 text-white border-emerald-600"
                  : "bg-white text-gray-600 border-gray-200 hover:border-emerald-300 hover:text-emerald-600"
              }`}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="w-3 h-3 flex items-center justify-center">
                  {showSortMenu
                    ? <i className="ri-arrow-up-s-line text-xs text-gray-400"></i>
                  ...
                  <div className="absolute right-0 top-full mt-1 bg-white border border-gray-100 rounded-xl shadow-lg z-20 min-w-[180px] overflow-hidden">
                    ...
                    {SORT_OPTIONS.map((opt) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-sm cursor-pointer transition-colors text-left ${
                          sortBy === opt.value
              ...
              <AnimatedSection key={job.id} variant="fade-up" delay={Math.min(index % 6, 5) * 70}>
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {job.tags.map((tag) => (
                      <span key={tag} className="bg-gray-50 border border-gray-100 text-gray-500 text-xs px-2 py-0.5 rounded-full">
                        {tag}
                      </span>
                    ))}
                  </div>

                  {/* Anonymous Notice + CTA */}
        ...
        {/* Bairro SEO Section — shown when filtering by neighborhood */}
        {selectedNeighborhood !== "Todos" && (
