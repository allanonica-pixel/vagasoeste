  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className={`p-6 border-b border-gray-100 dark:border-gray-800 ${isApprove ? "bg-emerald-50 dark:bg-emerald-950" : "bg-red-50 dark:bg-red-950"}`}>
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isApprove ? "bg-emerald-100 dark:bg-emerald-900" : "bg-red-100 dark:bg-red-900"}`}>
              <i className={`text-xl ${isApprove ? "ri-checkbox-circle-line text-emerald-600" : "ri-close-circle-line text-red-600"}`}></i>
            </div>
            <div>
              <h2 className={`text-lg font-bold ${isApprove ? "text-emerald-900 dark:text-emerald-300" : "text-red-900 dark:text-red-300"}`}>
                {isApprove ? "Aprovar pré-cadastro" : "Rejeitar pré-cadastro"}
              </h2>
              <p className={`text-sm ${isApprove ? "text-emerald-700 dark:text-emerald-400" : "text-red-700 dark:text-red-400"}`}>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Resumo do pré-cadastro</p>
            ...
            <div>
              ...
              {motivo === "Outro motivo" && (
                <textarea
                  ...
                  rows={3}
                  maxLength={300}
                  className="w-full border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-xl px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-red-400 resize-none"
                />
              )}
            </div>
          )}
          ...
          <div>
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Notificações automáticas</p>
            <div className="space-y-2">
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  onClick={() => setSendEmail(!sendEmail)}
                  className={`w-10 h-5 rounded-full transition-colors cursor-pointer relative ${sendEmail ? "bg-emerald-500" : "bg-gray-200 dark:bg-gray-700"}`}
                >
                ...
                <div>
                  <p className="text-sm text-gray-800 dark:text-gray-200 font-medium">Enviar email para {company.email}</p>
              ...
              {company.whatsapp && (
                <label className="flex items-center gap-3 cursor-pointer">
                  <div
                    ...
                    className={`w-10 h-5 rounded-full transition-colors cursor-pointer relative ${sendWhatsapp ? "bg-emerald-500" : "bg-gray-200 dark:bg-gray-700"}`}
                  >
                  ...
                  <div>
                    <p className="text-sm text-gray-800 dark:text-gray-200 font-medium">Notificar via WhatsApp ({company.whatsapp})</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Mensagem automática pelo WhatsApp Business</p>
                  </div>
                </label>
              )}
            </div>
          ...
          {/* Email Preview */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Prévia do email</p>
            <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
              <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Assunto: {emailPreview.subject}</p>
              <pre className="text-xs text-gray-600 dark:text-gray-400 whitespace-pre-wrap font-sans leading-relaxed">{emailPreview.body}</pre>
            </div>
          </div>
          ...
          {isApprove && (
            <div className="bg-emerald-50 dark:bg-emerald-950 border border-emerald-100 dark:border-emerald-900 rounded-xl p-4">
              <p className="text-emerald-800 dark:text-emerald-300 font-semibold text-sm mb-2 flex items-center gap-2">
        ...
        <div className="p-6 border-t border-gray-100 dark:border-gray-800 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 font-medium py-3 rounded-xl text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors whitespace-nowrap"
          >
            Cancelar
          </button>
          <button
