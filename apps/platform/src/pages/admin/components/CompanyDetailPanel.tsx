...
const JOB_STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  pausada: { label: "Pausada", color: "bg-gray-100 text-gray-600" },
  encerrada: { label: "Encerrada", color: "bg-red-100 text-red-600" },
  pendente: { label: "Pendente", color: "bg-amber-100 text-amber-700" },
};
...
export default function CompanyDetailPanel({ company, jobs, onApprove, onReject }: CompanyDetailPanelProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 sticky top-20 overflow-hidden">
      {/* Header */}
      <div className={`p-5 ${company.status === "pendente" ? "bg-amber-50 border-b border-amber-100" : "border-b border-gray-100"}`}>
        <div className="flex items-center gap-3 mb-3">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
            company.status === "pendente" ? "bg-amber-100" : company.status === "rejeitado" ? "bg-red-100" : "bg-emerald-100"
          }`}>
      ...
      <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
        ...
        <div className="bg-gray-50 rounded-xl p-3 space-y-1.5">
          <div className="flex justify-between text-xs">
            <span className="text-gray-500">Cadastro em</span>
            <span className="text-gray-800 font-medium">{company.registeredAt}</span>
          </div>
          {company.validadoEm && (
            <div className="flex justify-between text-xs">
              <span className="text-gray-500">Validado em</span>
              <span className="text-emerald-700 font-medium">{company.validadoEm}</span>
            </div>
          )}
          {company.motivoRejeicao && (
            <div className="pt-1 border-t border-gray-200">
              <p className="text-xs text-gray-500 mb-0.5">Motivo da rejeição</p>
              <p className="text-xs text-red-600">{company.motivoRejeicao}</p>
            </div>
          )}
        </div>
              ...
              {jobs.map((job) => (
                <div key={job.id} className="border border-gray-100 rounded-xl p-3">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className="font-semibold text-gray-800 text-xs leading-snug">{job.title}</p>
        ...
        {company.status === "ativo" && (
          <div className="flex gap-2 pt-2 border-t border-gray-100">
            <a
              ...
              className="flex-1 border border-gray-200 text-gray-600 font-medium py-2 rounded-lg text-xs cursor-pointer hover:bg-gray-50 transition-colors flex items-center justify-center gap-1.5 whitespace-nowrap"
            >
